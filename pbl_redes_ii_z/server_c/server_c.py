...
def ler_passagens_c():
    try:
        with open("pbl_redes_ii_z\\server_c\\passagens_c.json", "r", encoding="utf-8") as f:
            return json.load(f)
    except FileNotFoundError:
        return {}

@server_c.get("/ler-passagens")
...
def salvar_passagens_c(passagens_c):
    with open("pbl_redes_ii_z\\server_c\\passagens_c.json", "w", encoding="utf-8") as f:
        json.dump(passagens_c, f, indent=4)

@server_c.get("/passagens", response_model=List[passagem_c])
def listar_passagens_c():
    passagens_b = ler_passagens_c()
    return list(passagens_b.values())

@server_c.post("/passagens_c", response_model=passagem_c)
def adicionar_passagem_c(passagem_c: passagem_c):
    passagens_c = ler_passagens_c()

    # Verifica se a passagem_a já existe
    id_novo = str(len(passagens_c) + 1)
    if id_novo in passagens_c:
        raise HTTPException(status_code=400, detail="passagem_c já existe.")

    passagens_c[id_novo] = passagem_c.model_dump()
    salvar_passagens_c(passagens_c)
    return passagem_c


@server_c.get("/passagens_c", response_model=passagem_c)
async def buscar_passagem_por_id(id_server:str ,id_trecho: int):
    if id_server == 'c':
        try:
            # Abrir o arquivo JSON
            trechos = ler_passagens_c()
        ...
        if id_passagem == 'c':
            ...
            async with httpx.AsyncClient() as client:
                salvamento = await client.post(
                    f"http://localhost:8016/salvar-passagem",
...
async def consultar_voos():
    ...
    try:
        ...
        voos_a = []
    try:
        voos_b = await consultar_passagens_de(SERVER_B_URL)  # Servidor C
    except:
        voos_b = []
